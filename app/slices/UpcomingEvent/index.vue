...
watch(days, (n, o) => {
  if (n !== o) triggerBounce("day");
});
watch(hours, (n, o) => {
  if (n !== o) triggerBounce("hour");
});
watch(minutes, (n, o) => {
  if (n !== o) triggerBounce("minute");
});
watch(seconds, (n, o) => {
  if (n !== o) triggerBounce("second");
});

function triggerBounce(unit: "day" | "hour" | "minute" | "second") {
  if (unit === "day") {
    dayBounce.value = false;
    requestAnimationFrame(() => {
      dayBounce.value = true;
    });
  }
  if (unit === "hour") {
    hourBounce.value = false;
    requestAnimationFrame(() => {
      hourBounce.value = true;
    });
  }
  if (unit === "minute") {
    minuteBounce.value = false;
    requestAnimationFrame(() => {
      minuteBounce.value = true;
    });
  }
  if (unit === "second") {
    secondBounce.value = false;
    requestAnimationFrame(() => {
      secondBounce.value = true;
    });
  }
}
...
<template>
  ...
  >
    <div class="absolute top-0 right-0 w-1/3 h-full opacity-5 pointer-events-none">
      <p class="text-[20rem] font-black italic whitespace-nowrap leading-none">
        {{ slice.primary.background_watermark }}
      </p>
    </div>
    ...
    <div class="container mx-auto px-6 md:px-20 relative z-10">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        <div class="lg:col-span-4 mma-fade-left" :class="{ 'mma-active': isInView }">
          <h2 class="font-headline text-5xl font-black italic uppercase tracking-tighter mb-6">
            {{ slice.primary.title }}
          </h2>
          <div class="space-y-4">
            <div class="flex items-center gap-4 border-b border-outline-variant/15 pb-4">
              ...
              <p v-else class="text-lg font-bold uppercase">
                {{ asDate(slice.primary.event_date)?.toLocaleDateString() }}
              </p>
            </div>
            <div class="flex items-center gap-4 border-b border-outline-variant/15 pb-4">
          ...
          <div v-if="!slice.primary.tba" class="flex gap-4 mt-12">
            <div
              class="flex-1 bg-surface-container-highest p-4 text-center mma-scale-in mma-delay-1"
              :class="{ 'mma-active': isInView }"
            >
              <p
                class="text-4xl font-black font-headline text-white"
                :class="{ 'digit-bounce': dayBounce }"
              >
            ...
            <div
              class="flex-1 bg-surface-container-highest p-4 text-center mma-scale-in mma-delay-2"
              :class="{ 'mma-active': isInView }"
            >
              <p
                class="text-4xl font-black font-headline text-white"
                :class="{ 'digit-bounce': hourBounce }"
              >
            ...
            <div
              class="flex-1 bg-surface-container-highest p-4 text-center mma-scale-in mma-delay-3"
              :class="{ 'mma-active': isInView }"
            >
              <p
                class="text-4xl font-black font-headline text-white"
                :class="{ 'digit-bounce': minuteBounce }"
              >
        ...
        <div class="lg:col-span-8 mma-scale-in mma-delay-2" :class="{ 'mma-active': isInView }">
          <div class="bg-surface-container-highest p-1 border-2 border-primary-container/20">
            <div
              class="relative aspect-video flex items-center justify-center group cursor-pointer overflow-hidden"
            >
              <img
              ...
              />
              <div
                class="absolute inset-0 bg-background/40 group-hover:bg-background/20 transition-colors"
              />
              <div class="relative z-10 text-center px-4">
                <h3
                  class="font-headline text-4xl md:text-6xl font-black italic uppercase text-white drop-shadow-2xl"
                >
                  {{ slice.primary.main_headline }}
                </h3>
                <p class="text-primary font-black uppercase tracking-widest text-xl mt-2">
